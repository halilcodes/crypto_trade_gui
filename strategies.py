...
class Strategy:
    def __init__(self, client, contract:Contract, exchange: str, timeframe: str, balance_pct:float,
        ...
        if self.exchange.lower() == "binance":
            self.candles = self.client.get_historical_candles(self.contract, self.tf)[-5:]
    ...
    def parse_trades(self, price:float, size: float, timestamp: int):
        ...
        # Missing Candles:
            ...
            logger.info("%s missing %s for %s %s", self.exchange, missing_candles, self.contract.symbol,
                        self.tf)
...
class TechnicalStrategy(Strategy):
    ...
    def _check_signal(self):
        ...
        print(f"{rsi = }/ {macd_line = } / {macd_signal = }")
        print(f"{rsi }/ {macd_line } / {macd_signal }")
